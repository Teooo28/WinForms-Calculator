...
namespace Calculator
{
    public partial class Form1 : Form
    {
        ...
        private void bMod_Click(object sender, EventArgs e)
        {
            ...
            string[] op = s.Split('+', '-', '*', '/', '%');
            op[2].Remove(op[2].Length - 1);
            double nr1 = Convert.ToDouble(op[1]);
            double nr2 = Convert.ToDouble(op[2]);
            int p = 0;
            int p1 = s.IndexOf('-'); if (p1 != -1) p = p1;
            int p2 = s.IndexOf('+'); if (p2 != -1) p = p2;
            int p3 = s.IndexOf('*'); if (p3 != -1) p = p3;
            int p4 = s.IndexOf('/'); if (p4 != -1) p = p4;
            s = s.Remove(p + 1);
            textBox1.Text +=  Convert.ToString(nr2 / 100 * nr1);
        }
